/*
 * Copyright 2012-2013 University of Maryaland. All rights reserved.
...
template <class dataType, class realType> inline __device__ void modulateAndNormalizeMany_kernel(
    ...
    {
        // Output coordinates
        const int idxRow = idx / dataW;
        const int idxCol = idx % dataW;

        int idxLinear = LINPOS(idxRow,idxCol,dataW);
        const int kernelSize = dataW * dataH;
...
extern "C" __global__ void modulateAndNormalizeManyFC(
    ...
    modulateAndNormalizeMany_kernel<fComplex,float>(n, offset, d_Dst, d_SrcData, d_SrcKernel, dataW, dataH, numElem, c);
}
...
extern "C" __global__ void modulateAndNormalizeManyDC(
    ...
    modulateAndNormalizeMany_kernel<dComplex,double>(n, offset, d_Dst, d_SrcData, d_SrcKernel, dataW, dataH, numElem, c);
}
#endif
...
////////////////////////////////////////////////////////////////////////////////
// Calculate vector norm of the difference between 2 vectors within 
// 256 samples (source can be either real or complex, but result is always real)
////////////////////////////////////////////////////////////////////////////////
template <class dataType, class realType> inline __device__ void reduceNormErr256_kernel(
    int n,
    int offset,
    realType* d_Dst,
    const dataType* d_SrcA,
    const dataType* d_SrcB,
    realType p
){
    __shared__ realType sum_buffer[256];
    const unsigned int idx = BLOCK_DIM1D * blockIdx.x + threadIdx.x + offset;
    ...
    // Load absolute value of the error from memory to buffer and take pth power
    if( inRange )
        sum_buffer[i] = pow(fabs(d_SrcB[idx] - d_SrcA[idx]), p);
    __syncthreads();
...
extern "C" __global__ void reduceNormErr256F(
    int n,
    int offset,
    float* d_Dst,
    const float* d_SrcA,
    ...
    reduceNormErr256_kernel<float,float>(n, offset, d_Dst, d_SrcA, d_SrcB, p );
}

extern "C" __global__ void reduceNormErr256FC(
    int n,
    int offset,
    float* d_Dst,
    const fComplex* d_SrcA,
    const fComplex* d_SrcB,
    float p
){
    reduceNormErr256_kernel<fComplex,float>(n, offset, d_Dst, d_SrcA, d_SrcB, p );
}

#if __CUDA_ARCH__ >= 200
extern "C" __global__ void reduceNormErr256D(
    int n,
    int offset,
    double* d_Dst,
    const double* d_SrcA,
    ...
    reduceNormErr256_kernel<double,double>(n, offset, d_Dst, d_SrcA, d_SrcB, p );
}

extern "C" __global__ void reduceNormErr256DC(
    int n,
    int offset,
    double* d_Dst,
    const dComplex* d_SrcA,
    const dComplex* d_SrcB,
    double p
){
    reduceNormErr256_kernel<dComplex,double>(n, offset, d_Dst, d_SrcA, d_SrcB, p );
}
#endif
...
template <class dstType, class srcType> inline __device__ void zeroPadR2C_kernel(
    ...
    if ((idx - offset) < n)
    {
        ...
        {
            dstType val = {0};
            if( x < srcDimX && y < srcDimY && z < srcDimZ )
            {
                val.x = d_SrcReal[z*srcDimX*srcDimY + y*srcDimX + x];
            }
...
extern "C" __global__ void zeroPadF2FC(
    ...
    zeroPadR2C_kernel<fComplex,float>(n, offset, d_DstComplex, dstDimX, dstDimY, dstDimZ, d_SrcReal, srcDimX, srcDimY, srcDimZ );
}
...
extern "C" __global__ void zeroPadF2DC(
    ...
    zeroPadR2C_kernel<dComplex,float>(n, offset, d_DstComplex, dstDimX, dstDimY, dstDimZ, d_SrcReal, srcDimX, srcDimY, srcDimZ );
}
...
extern "C" __global__ void zeroPadD2DC(
    ...
    zeroPadR2C_kernel<dComplex,double>(n, offset, d_DstComplex, dstDimX, dstDimY, dstDimZ, d_SrcReal, srcDimX, srcDimY, srcDimZ );
}
...
extern "C" __global__ void zeroPadD2FC(
    ...
    zeroPadR2C_kernel<fComplex,double>(n, offset, d_DstComplex, dstDimX, dstDimY, dstDimZ, d_SrcReal, srcDimX, srcDimY, srcDimZ );
}
#endif
...
extern "C" __global__ void prepareMyerFiltersFC(
    ...
    prepareMyerFilters_kernel<fComplex,float>(n, offset, d_Filter, width, numDir );
}
...
extern "C" __global__ void prepareMyerFiltersDC(
    ...
    prepareMyerFilters_kernel<dComplex,double>(n, offset, d_Filter, width, numDir );
}
#endif
