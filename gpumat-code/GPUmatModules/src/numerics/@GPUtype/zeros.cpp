...
/*
 * ZEROS(N, GPUsingle) is the N-by-N zeros matrix.
 * ZEROS(N, GPUdouble) is the N-by-N zeros matrix
 *
 * ZEROS(M,N, GPUsingle) or ZEROS([M,N], GPUsingle) is an M-by-N matrix with zeros
 *
 */
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (gm->comp.getCompileMode() == 1) {
    ...
    gm->comp.functionStart("GPUMAT_mxZerosDrv");
    ...
    GPUtype R = gm->gputype.mxZerosDrv(IN, nrhs-1, prhs);
    plhs[0] = gm->gputype.createMxArray(R);
  }
