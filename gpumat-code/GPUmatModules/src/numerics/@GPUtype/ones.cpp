...
/*
 * ONES(N, GPUsingle) is the N-by-N ones matrix.
 * ONES(N, GPUdouble) is the N-by-N ones matrix
 *
 * ONES(M,N, GPUsingle) or ONES([M,N], GPUsingle) is an M-by-N matrix with ones
 *
 */
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (gm->comp.getCompileMode() == 1) {
    ...
    gm->comp.functionStart("GPUMAT_mxOnesDrv");
    ...
    GPUtype R = gm->gputype.mxOnesDrv(IN, nrhs-1, prhs);
    plhs[0] = gm->gputype.createMxArray(R);
  }
