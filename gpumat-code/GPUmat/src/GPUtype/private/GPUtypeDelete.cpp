...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int slot = (int ) mxGetScalar(prhs[0]);
  if (slot==-1)
      return;

  GPUtype *p = (GPUtype *) GPUman->extCacheGetGPUtypePtrBySlot(slot);
  //mexPrintf("%p\n", );
  //mexPrintf("%p\n",mxID(prhs[0]));
  //mexPrintf("Slot %d -> %p\n",slot, p);
  if (p==NULL)
  ...
  // Remove mx from extCache
  GPUman->extCacheCleanPtrBySlot(slot);
  ...
  //plhs[0] = mxCreateDoubleScalar((unsigned int) p->getNumel());
