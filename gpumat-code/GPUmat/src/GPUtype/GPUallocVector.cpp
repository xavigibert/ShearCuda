...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (GPUman->getCompileMode()==1) {
	...
	CUcontext *cuContext = GPUman->getContext();
	CUresult err = cuCtxPushCurrent(*cuContext);
	if (err != CUDA_SUCCESS) {
		mexErrMsgTxt("Unable to attach GPU context.");
	}*/
	...
	//mexPrintf("%d\n",*cuContext);
	//CUresult err = cuCtxPushCurrent(*cuContext);
	//CUresult err = cuCtxAttach(cuContext,0);
	...
	//err = cuCtxDetach(*cuContext);

	/*if (err != CUDA_SUCCESS) {
		mexErrMsgTxt("Unable to detach GPU context.");
	}*/
	//err = cuCtxPopCurrent(NULL);
	/*	if (err != CUDA_SUCCESS) {
			mexErrMsgTxt("Unable to detach GPU context.");
