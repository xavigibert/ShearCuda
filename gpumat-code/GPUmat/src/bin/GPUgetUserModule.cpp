            ...
            int nrhs,  const mxArray *prhs[] )
{
  ...
  try {
    int modnumber = 0;
    modnumber = GPUman->getUserModuleNumberByName(modname);
    if (modnumber<0) {
      mexErrMsgTxt("Unable to find specified module. Please check module name.");
    }
    ...
    // load function
    //CUfunction drvfun;
    //CUresult status = cuModuleGetFunction(&drvfun, mymod->cuModule, "mykernel");
  ...
  plhs[0] = mxCreateDoubleScalar(UINTPTR mod);
}
