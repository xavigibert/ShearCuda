             ...
             int nrhs,  const mxArray *prhs[] )
{
  ...
  strlen = mxGetM(prhs[1]);
	...
	char *funname = (char*) Mymalloc((strlen+1)*sizeof(char),&mygc);
	memset(funname,0,strlen+1);
	mxGetString(prhs[1], funname, strlen+1);


	CUfunction *drvfun;
	try {
		drvfun = GPUman->getUserFunctionByName(modname, funname);
	} catch (GPUexception ex) {
		mexErrMsgTxt(ex.getError());
	}

	plhs[0] = mxCreateDoubleScalar(UINTPTR drvfun);
}
